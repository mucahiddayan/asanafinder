...
function reportBug(){
	...
	window.open(pluginSettings.homeURL+'/?p='+pluginSettings.bugreport_page,'Report Bug','width=500 height=500 left='+($width/2-250)+' top=200');
}
...
app.controller('mainCtrl', ['$scope', '$http','$location','$filter','$compile','$timeout', function ($scope, $http,$location,$filter,$compile,$timeout ) {
	...
	$scope.SPA = $scope.SFA = $scope.SPFA = false;
	...
	//REPORT BUG
	$scope.canReportBug = !isEmpty(pluginSettings.bugreport_page);	
	...
	__ng.sudo = function(msg){
		...
		return parseInt(result) === 1?{scope:$scope,http:$http,location:$location,filter:$filter}:{};
	...
	$scope.saveAsanas = function(){				
		...
		if (!$scope.isGast && $scope.selectedAsanas.length) {
			$http({
				...
				if (response.status == 403) {
					__cE('No access');
					__mb.warn('Sie besitzen nicht genügend Rechte um die Sequenz speichern können!');
				}
			});
			__ls.setItem(__ls.getItem('yogamaker').toLowerCase()+'.asanas',angular.toJson($scope.selectedAsanas));					
		}else{
	...
	$scope.$watchGroup(Object.keys($scope.toWatch).map(function(obj){return obj.replace('_','.')}),function(newV,oldV){	
		...
		// in progress
		$scope.preparingAsanas = ($scope.levelFilter || $scope.typeFilter)?$scope.substract($scope.getPreparingAsanasBySearch($scope.searchFilter),$scope.prepareFilter($scope.paginated)):$scope.getPreparingAsanasBySearch($scope.searchFilter);
		$scope.followingAsanas = ($scope.levelFilter || $scope.typeFilter)?$scope.substract($scope.getFollowingAsanasBySearch($scope.searchFilter),$scope.prepareFilter($scope.paginated)):$scope.getFollowingAsanasBySearch($scope.searchFilter);
		$scope.mutualAsanas = ($scope.levelFilter || $scope.typeFilter)?$scope.substract($scope.getMutualAsanasBySearch($scope.searchFilter),$scope.prepareFilter($scope.paginated)):$scope.getMutualAsanasBySearch($scope.searchFilter);
		

		/*
		$scope.preparingAsanas = $scope.getPreparingAsanasBySearch($scope.searchFilter);
		$scope.followingAsanas = $scope.getFollowingAsanasBySearch($scope.searchFilter);
		$scope.mutualAsanas = $scope.getMutualAsanasBySearch($scope.searchFilter);
		*/
	});

	$scope.prepareFilter = function(arr){
		return $filter('allFilter')(arr,$scope.sortType,$scope.sortReverse,$scope.levelFilter,$scope.typeFilter,$scope.searchFilter,$scope.XOR);
	}

	$scope.substract = function (arr,subs){
		return arr.filter(function(el){
			return subs.every(function(obj){
				return el !== obj;
			});
		});
	}
...
//++++++++++++++++++++++++++++++ filter functions
	...
	$scope.dragStarted = function(event){
		if (event.dataTransfer.setDragImage) {		
		 ...
		 console.log("drag image is not sported ");
	   }
	...
	$scope.getPreparingAsanas = function(asanaTitle){
		return $filter('filter')($scope.posts,{preparings:asanaTitle,followings:'!'+asanaTitle});
	}

	$scope.getFollowingAsanas = function(asanaTitle){
		return $filter('filter')($scope.posts,{followings:asanaTitle,preparings:'!'+asanaTitle});
	}

	$scope.getMutualAsanas = function(asanaTitle){
		return $filter('filter')($scope.posts,{preparings:asanaTitle,followings:asanaTitle});
	}

	$scope.getMutualAsanasBySearch = function(search){
		if (isEmpty(search)) {return [];}
		var tags = customSplit(search);
		var result = [];
		if (angular.isArray(tags)) {
			tags.map(function(tag){
				$.extend(result,$scope.getMutualAsanas(tag));
			});
			return $filter('unique')(result,'title');
		}
		else{
			return $scope.getMutualAsanas(tags);
		}
	}

	$scope.getPreparingAsanasBySearch = function(search){
		if (isEmpty(search)) {return [];}
		var tags = customSplit(search);
		var result = [];
		if (angular.isArray(tags)) {
			tags.map(function(tag){
				$.extend(result,$scope.getPreparingAsanas(tag));
			});
			return $filter('unique')(result,'title');
		}
		else{
			return $scope.getPreparingAsanas(tags);
		}
	}

	$scope.getFollowingAsanasBySearch = function(search){
		if (isEmpty(search)) {return [];}
		var tags = customSplit(search);
		var result = [];
		if (angular.isArray(tags)) {
			tags.map(function(tag){
				$.extend(result,$scope.getFollowingAsanas(tag));
			});
			return $filter('unique')(result,'title');
		}
		else{
			return $scope.getFollowingAsanas(tags);
		}
	}
}]);
...
app.filter('allFilter', function($filter) {
	...
	return function(input,sortby,direction,levFil,typFil,search,xor,scope) {
		...
		var filtered = $filter('orderBy')(input,sortby,direction);
			if (xor) {
				...
				filtered = $filter('multiSearchFilter')(filtered,search,scope);
			}else{
				search = customSplit(search);
				filtered = $filter('filter')(filtered,levFil).concat($filter('filter')(filtered,typFil));
				if (!isEmpty(search)) {
					filtered = filtered.concat($filter('concatFilter')(input,search));
				}					
			}			
		return $filter('unique')(filtered,'title');
	};
});

function getAsanasByTitle(filter,asanas,tag,scope){
	var angFilter = filter('filter'),	
	result = angFilter(asanas, {title:tag});	

	return {
		unique:filter('unique')(result,'title'),		
	};
}

app.filter('concatFilter', function($filter) {
	return function(asanas,tags,filterType,uniq,scope) {
		var result = [];
		var uniq = isEmpty(uniq)?true:uniq;     
		var angFilter = isEmpty(filterType)?$filter('filter'):$filter(filterType);
		if (!angular.isArray(tags)) {
			result = result.concat(getAsanasByTitle($filter,asanas,tags,scope).unique);	
		}else{
			tags.some(function(tag){
				if(!isEmpty(tag)){
					result = result.concat(getAsanasByTitle($filter,asanas,tag,scope).unique);					
				}
			});
		}
...
app.filter('multiSearchFilter', function($filter) {
	var angFilter = $filter('filter');
	return function(asanas,tags,scope){
		tags = customSplit(tags);
		if (!angular.isArray(tags)){
			return getAsanasByTitle($filter,asanas,tags,scope).unique;
		} 
		else{			
			return $filter('concatFilter')(asanas,tags,'filter',true,scope);
		}
	}	
});
...
app.directive('ngEnter', function () {
    return function (scope, element, attrs) {
        element.bind("keydown keypress keyup", __helper.debounce(function(){
        	...
        	if (event.which === 13 || event.key == __seperator || event.which === 8) {
                scope.$apply(function () {
                    scope.$eval(attrs.ngEnter);
                    scope.searchAll();
                });
...
app.directive("searchNow", function($timeout) {
	return {
		restrict: 'A',
		require: 'ngModel',
		link: function (scope, elem, attrs, ngModelCtrl) {
			elem.bind("keydown keypress blur keyup",function(e) {
				/*$timeout(function(){
